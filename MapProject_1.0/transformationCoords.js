...
function toDEgMinSec(coordinate){
  ...
  function convertDMS(lat, lng) {
	var latitude = toDEgMinSec(lat);
	var latitudeCardinal = lat >= 0 ? "N" : "S";
  
	var longitude = toDEgMinSec(lng);
  ...
  function toDEgDecimal(coordinate){
	var absolute = Math.abs(coordinate);
	var degrees = Math.floor(absolute);
	var minutesNotTruncated = (absolute - degrees) * 60;
	
	return degrees+ 'º ' + " " + minutesNotTruncated.toFixed(3);
   
  
  }
  
  function convertDM(lat, lng) {
	var latitude = toDEgDecimal(lat);
	var latitudeCardinal = lat >= 0 ? "N" : "S";
  
	var longitude = toDEgDecimal(lng);
	var longitudeCardinal = lng >= 0 ? "E" : "W";
  ...
  function absoluteLatLon(lat, lng) {
	var latitude = lat; 
	var longitude = lng;
	...
	return latitudeCardinal +" " + latitude.toFixed(5) + " | " + longitudeCardinal + " " + longitude.toFixed(5);
  }
  
  
  function flOverall(f,l, code){
  
	//set code = DM eg. N38 54.658 Ε021 43.567
	//set code = DMS eg. N38 23 12 E023 43 18
	//set code = absolute eg. 38.67890 23.67893
  
	var finalCoord; 
	switch(code){
  
	  case "DMS":
		finalCoord = convertDMS(f,l);
		break; 
	  
	  case "DM":
		finalCoord = convertDM(f,l);
		break; 
  
	  case "absolute":
		finalCoord = absoluteLatLon(f,l);
		break
